...
@Component({
  selector: 'app-penetapan-ac-add-detail',
  templateUrl: './penetapan-ac-add-detail.component.html',
  styleUrls: ['./penetapan-ac-add-detail.component.css'],
  providers: [PenetapanService, ReferensiTemplateService]
})
export class PenetapanAcAddDetailComponent implements OnInit {
  nomorstring:string;
          ...
          if (accept) {
            ...
            this._service.getNomorTerakhir().subscribe(result => {
                ...
                if(this.nomorPenetapan != null){
                  ...
                  this.selected = [];
                  this.router.navigate(['/penetapanac']);
                  this.prepareTableContent();
            });

            this._refTemplateService.checkTemplate(8).subscribe(result =>{
              if(!result){
                this.snackbar.showSnackBar("error", "Gagal Upload Template Penetapan AC");
              }else{
                console.log("Sukses");
              }
            });
  ...
  prepareTableContent() {
    this._service.getDaftarBerkasAC().subscribe(result => {
    this.items = result;
