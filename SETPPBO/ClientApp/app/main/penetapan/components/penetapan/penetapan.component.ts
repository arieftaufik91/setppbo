...
@Component({
  ...
  styleUrls: ['./penetapan.component.css'],
  providers: [PenetapanService, AdministrasiBandingGugatanService],
})
export class PenetapanComponent implements OnInit {
  items: Penetapan[];
  tempItems: Penetapan[];
  item: Penetapan;
  cetak :string;
  ...
  prepareTableContent() {
    this._service.getPenetapan().subscribe(result => {
      this.items = result;
  ...
  onAddClick() {
    ...
    this.router.navigate(['/penetapanadd']);
  ...
  onEditClick() {
    ...
    this.noPenetapan= this.noPenetapan.replace(new RegExp("/", 'g'), "");
    this.router.navigate(['/penetapanedit', this.noPenetapan]);
