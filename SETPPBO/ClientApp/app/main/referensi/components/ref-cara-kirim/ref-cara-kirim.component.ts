...
import { RefCaraKirimFormComponent } from '../ref-cara-kirim-form/ref-cara-kirim-form.component';
...
@Component({
  selector: 'app-ref-cara-kirim',
  templateUrl: './ref-cara-kirim.component.html',
  styleUrls: ['./ref-cara-kirim.component.css'],
...
export class RefCaraKirimComponent implements OnInit {
  // default properties
  items: Refcarakirimpermohonan[];
  tempItems: Refcarakirimpermohonan[];
  item: Refcarakirimpermohonan;
   ...
   */
  prepareTableContent() {
    this._service.getAllDaftar().subscribe(result => {
      this.items = result;
  ...
  onAddClick() {
    // add new config
    //dialog form
    let dialogRef = this.dialog.open(RefCaraKirimFormComponent, {});
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result != null) {
        this._service.update(result).subscribe(result => {
          this.prepareTableContent();
          this.snackbar.showSnackBar("success", "Edit Cara Kirim berhasil!");
          this.deactivateState();
          this.selected = [];
        });
      }
  ...
  onDeleteClick() {
    // konfirmasi
    this.dialogsService
      .confirm("Konfirmasi", "Yakin mau menghapus Ref Cara Kirim ini?")
      .subscribe(accept => {
        if (accept) {
          console.log(this.selected[0]);
          let item = this.selected[0];
          this._service.delete(this.selected[0].RefCaraKirimId).subscribe(
            result => {
              this.prepareTableContent();
