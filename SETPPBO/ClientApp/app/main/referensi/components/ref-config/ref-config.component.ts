...
import { RefConfigFormComponent } from '../ref-config-form/ref-config-form.component';
...
@Component({
  selector: 'ref-config',
  templateUrl: './ref-config.component.html',
  styleUrls: ['./ref-config.component.css'],
...
export class RefConfigComponent implements OnInit {
  ...
  onAddClick() {
    // add new config
    //dialog form
    let dialogRef = this.dialog.open(RefConfigFormComponent, {});
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result != null) {
        this._service.update(result).subscribe(result => {
          this.prepareTableContent();
          this.snackbar.showSnackBar("success", "Edit Config berhasil!");
          this.deactivateState();
          this.selected = [];
        });
      }
