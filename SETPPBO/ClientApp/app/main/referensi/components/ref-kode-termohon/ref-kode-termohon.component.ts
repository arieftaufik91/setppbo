...
import { RefKodeTermohonService } from '../../services/ref-kode-termohon.service';

import { Router } from "@angular/router";
import { DatatableComponent } from "@swimlane/ngx-datatable";
import { MatDialog } from "@angular/material";

import { RefKodeTermohonFormComponent } from '../ref-kode-termohon-form/ref-kode-termohon-form.component';
...
@Component({
    selector: 'ref-kode-termohon',
    templateUrl: './ref-kode-termohon.component.html',
    styleUrls: ['./ref-kode-termohon.component.css'],
...
export class RefKodeTermohonComponent implements OnInit {
    // default properties
    items: RefKodeTermohon[];
    tempItems: RefKodeTermohon[];
    item: RefKodeTermohon;
    ...
    constructor(
        private _service: RefKodeTermohonService,
    ...
    ngOnInit() {
        ...
        this.columns = [
            { prop: "OrganisasiId" },
            { prop: "KodeOrganisasi" },
             { prop: "UraianOrganisasi" },
             { prop: "UraianLengkapOrganisasi" },
             { prop: "UraianJabatan" },
             { prop: "KodeSatker" },
             { prop: "Alamat" },
             { prop: "KodeSurat" },
    ...
    onAddClick() {
        // add new alasan
        //dialog form
        let dialogRef = this.dialog.open(RefKodeTermohonFormComponent, {});
        ...
        dialogRef.afterClosed().subscribe(result => {
            if (result != null) {
                this._service.update(result).subscribe(result => {
                    this.prepareTableContent();
                    this.snackbar.showSnackBar("success", "Edit Kode Termohon berhasil!");
                    this.deactivateState();
                    this.selected = [];
                });
            }
