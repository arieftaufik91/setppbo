...
import { RefPenomoranFormComponent } from '../ref-penomoran-form/ref-penomoran-form.component';
...
@Component({
  selector: 'app-ref-penomoran',
  templateUrl: './ref-penomoran.component.html',
  styleUrls: ['./ref-penomoran.component.css'],
...
export class RefPenomoranComponent implements OnInit {
  // default properties
  items: RefPenomoran[];
  tempItems: RefPenomoran[];
  item: RefPenomoran;
  ...
  ngOnInit() {
    this.prepareTableContent();
        ...
        // specify column
        this.columns = [
            { name: "Jenis Penomoran",  prop: "RefJenisPenomoran.Uraian" },
            { name: "Tahun",  prop: "Tahun" },
            { name: "OrganisasiId",  prop: "OrganisasiId" },
            { name: "KodeOrganisasi",  prop: "KodeOrganisasi" },
            { name: "NamaOrganisasi",  prop: "NamaOrganisasi" },
            { name: "NomorTerakhir",  prop: "NomorTerakhir" }
        ];
  }
  ...
  onAddClick() {
    // add new quote
    //dialog form
    let dialogRef = this.dialog.open(RefPenomoranFormComponent, {});
   ...
   dialogRef.afterClosed().subscribe(result => {
      ...
      if (result != null) {
        this._service.update(result).subscribe(result => {
          this.prepareTableContent();
          this.snackbar.showSnackBar("success", "Edit Penomoran berhasil!");
       });
      }
    });
  }
  ...
  onDeleteClick() {
    // konfirmasi
    this.dialogsService
      .confirm("Konfirmasi", "Yakin mau menghapus Penomoran?")
      .subscribe(accept => {
        if (accept) {
          console.log(this.selected[0]);
          let item = this.selected[0];
          this._service.delete(this.selected[0].RefNormaWaktuId).subscribe(
            result => {
              this.prepareTableContent();
              this.snackbar.showSnackBar("success", "Hapus Penomoran berhasil!");
            },
            error => {},
            () => {}
          );
