...
import { RefjenisnormawaktuFormComponent } from '../referensi-jenisNormaWaktu-form/referensi-jenisNormaWaktu-form.component';

@Component({
  selector: 'app-referensi-jenisNormaWaktu',
  templateUrl: './referensi-jenisNormaWaktu.component.html',
  styleUrls: ['./referensi-jenisNormaWaktu.component.css',
],
...
export class RefjenisnormawaktuComponent implements OnInit {
  items: Refjenisnormawaktus[];
  tempItems: Refjenisnormawaktus[];
  item: Refjenisnormawaktus;
  ...
  state = "inactive";
  constructor(
    private _service: RefjenisnormawaktuService,
  ...
  ngOnInit() {
    ...
    this.columns = [{ prop: "RefJenisNormaWaktuId" }, {prop: "Uraian"}];
  }
  ...
  onAddClick() {
    // add new quote
    //dialog form
    let dialogRef = this.dialog.open(RefjenisnormawaktuFormComponent, {});
    ...
    this.dialogsService
      ...
      .subscribe(accept => {
        if (accept) {
          ...
          this._service.delete(this.selected[0].RefJenisNormaWaktuId).subscribe(
            result => {
              this.prepareTableContent();
